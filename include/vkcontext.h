...
struct SubMesh {
	...
	glm::vec3 bbox_min = glm::vec3(FLT_MAX);
	glm::vec3 bbox_max = glm::vec3(-FLT_MAX);

	glm::vec3 get_center() const { return (bbox_min + bbox_max) * 0.5f; }
};
...
public:
	...
	// boundary box
	glm::vec3 get_bbox_min() const;
	glm::vec3 get_bbox_max() const;
...
public:
	...
	const std::vector<Entity*>& get_visible_entities();
	...
	Buffer<Material>& get_unique_materials_buffer(Semaphore& timeline_semaphore, bool force_update = false);
	Buffer<Light>& get_lights_buffer(Semaphore& timeline_semaphore, bool force_update = false);
	Buffer<glm::mat4>& get_model_matrices_buffer(Semaphore& tl_semaphore);
	Buffer<MaterialTexIDs>& get_scene_texIDs_buffer(Semaphore& tl_semaphore, bool force_update = false);
...
private:
	...
	std::unique_ptr<Buffer<Material>> unique_materials_buffer = nullptr;
	std::unique_ptr<Buffer<Light>> lights_buffer = nullptr;
	std::unique_ptr<Buffer<glm::mat4>> model_matrices_buffer = nullptr;
	std::unique_ptr<Buffer<MaterialTexIDs>> scene_texIDs_buffer = nullptr;
...
private:
	...
	std::vector<std::unique_ptr<PushConstants>> m_push_constants;
	std::vector<std::unique_ptr<DescriptorSet>> m_descriptor_set;
	...
	std::vector<Semaphore> image_available_semaphore;
	std::vector<Semaphore> render_finished_semaphore;
...
void Mesh::load_obj_textures_and_materials(
	...
	for (const auto& obj_mat : materials_tinyobj) {
		...
		if (materials.back().texIDs.base_color_tex_id == INVALID_ID) { materials.back().texIDs.base_color_tex_id = default_texID.white_srgb; }
		...
		for (const auto& primitive : gltf_mesh.primitives) {
			...
			if (attribs.find("POSITION") == attribs.end()) {
				...
				if (accessor.minValues.size() == 3) {
					submesh.bbox_min = glm::vec3(accessor.minValues[0], accessor.minValues[1], accessor.minValues[2]);
					this->bbox_min = glm::min(this->bbox_min, submesh.bbox_min);
				}
				if (accessor.maxValues.size() == 3) {
					submesh.bbox_max = glm::vec3(accessor.maxValues[0], accessor.maxValues[1], accessor.maxValues[2]);
					this->bbox_max = glm::max(this->bbox_max, submesh.bbox_max);
				}
			}
...
glm::vec3 Mesh::get_bbox_min() const { return this->bbox_min; }
glm::vec3 Mesh::get_bbox_max() const { return this->bbox_max; }
glm::vec3 Mesh::get_bbox_center() const { return (this->bbox_min + this->bbox_max) * 0.5f; }
...
void Mesh::make_solid(bool mesh_is_solid) { this->is_solid = mesh_is_solid; }
...
glm::mat4 Entity::get_model_matrix() {
	if (first_update) {
		...
		last_update = std::chrono::high_resolution_clock::now(); // reset timer

		float speed = glm::length(lin_velocity);
		float_t drag_coeff = this->mesh->get_drag_coefficient();
		float_t mass = this->mesh->get_mass_kg() * (scale.x * scale.y * scale.z);
		mass = glm::max(mass, 0.0001f);
		glm::vec3 dim = (this->mesh->get_bbox_max() - this->mesh->get_bbox_min()) * scale;

		// === LINEAR PHYSICS ===
		if (lin_physics_enabled) {
			glm::vec3 total_force =
				(gravity_enabled ? (gravity * mass) : glm::vec3{ 0.0f, 0.0f, 0.0f }) +
				lin_thrust;
			if (speed > 0.0001) {
				// Get local velocity by inverting the rotation
				...
				glm::vec3 local_v = glm::vec3(R * glm::vec4(lin_velocity, 0.0f));
				glm::vec3 dir = glm::abs(glm::normalize(local_v));
				...
				float max_drag = (speed * mass) / sec;
				drag_mag = glm::min(drag_mag, max_drag);
				glm::vec3 drag_direction = -glm::normalize(lin_velocity);
				total_force += drag_direction * drag_mag;
			}
			glm::vec3 lin_acceleration = total_force / mass;
			lin_velocity += lin_acceleration * sec;
		}
		else {
			lin_velocity += lin_thrust * sec;
		}
		...
		if (rot_physics_enabled) {
			...
			inertia.x = (1.0f / 12.0f) * mass * (dim.y * dim.y + dim.z * dim.z);
			inertia.y = (1.0f / 12.0f) * mass * (dim.x * dim.x + dim.z * dim.z);
			inertia.z = (1.0f / 12.0f) * mass * (dim.x * dim.x + dim.y * dim.y);

			// Make sure inertia components never reach zero
			inertia.x = glm::max(inertia.x, 0.0001f);
			inertia.y = glm::max(inertia.y, 0.0001f);
			inertia.z = glm::max(inertia.z, 0.0001f);

			glm::vec3 spin_acceleration = spin_thrust / inertia;

			if (tumbling_enabled && speed > 1.0f) {
				float dynamic_pressure = 0.5f * this->rho * (speed * speed);
				float area = (dim.y * dim.z) + (dim.x * dim.z) + (dim.x * dim.y); // total surface approx
				glm::vec3 tumble_axis = glm::cross(glm::normalize(lin_velocity), glm::vec3(0, 1, 0));
			...
			if (angular_speed > 0.0001f) {
				...
				glm::vec3 rot_drag_dir = -glm::normalize(spin_velocity);
				glm::vec3 torque = rot_drag_dir * rot_drag_mag;
				spin_acceleration += torque / inertia;
			}
			spin_velocity += spin_acceleration * sec;
		}
		else {
			spin_velocity += spin_thrust * sec;
		}
		...
		static const float_t MIN_SCALE = 0.0001;
		if (scale.x <= MIN_SCALE) { scale.x = MIN_SCALE; this->visible = false; }
		if (scale.y <= MIN_SCALE) { scale.y = MIN_SCALE; this->visible = false; }
		if (scale.z <= MIN_SCALE) { scale.z = MIN_SCALE; this->visible = false; }
	...
	// calculate model matrix with updated transformation
...
Entity& Entity::operator=(Entity&& other) noexcept {
	...
	if (this != &other) {
		...
		rotation = other.rotation;
		scale = other.scale;
		unique_ID = other.unique_ID; other.unique_ID = INVALID_ID;
		mesh = other.mesh; other.mesh = nullptr;
...
Entity& Entity::operator=(const Entity& other) {
	...
	if (this != &other) {
		...
		rho = other.rho;
		tumble_strength = other.tumble_strength;
		first_update = true; // Reset timer for the "new" state of this object
	}
	else {
		Log::debug("... 'this' and '&other' are identical! Returning '*this' (unmodified).");
	}
...
const std::vector<Entity*>& Scene::get_visible_entities() {
	this->visible_entities.reserve(this->entities.size());
	this->visible_entities.clear();
	for (auto& entity : this->entities) {
		if (entity.second->is_visible()) {
			visible_entities.push_back(entity.second.get());
		}
	}
...
// returns an SSBO holding the properties of the unique materials of the Scene;
// note: this buffer will only update if
// (1) the number of unique materials has changed
// (2) this is the first call for the given Scene object
// (3) the 'force_update' flag is set to 'true'
Buffer<Material>& Scene::get_unique_materials_buffer(Semaphore& timeline_semaphore, bool force_update) {
	uint32_t num_materials = static_cast<uint32_t>(this->unique_materials.size());
	VulkanManager& manager = VulkanManager::get_singleton();

	// check if the unique_materials_buffer SSBO needs updating
	if (this->unique_materials_buffer) {

		// return the existing buffer if the number of unique materials hasn't changed
		if (this->unique_materials_buffer->get_elements() == num_materials) {
			if (!force_update) {
				return *this->unique_materials_buffer;
			}
		}
		// release the old SSBO in case the number of materials has changed
		else {
			manager.get_device().wait_idle();
			this->unique_materials_buffer.reset(nullptr); // clean-up the old resource before proceeding
		}
	}
	
	// create unique Materials SSBO in case it doesn't yet exist
	if (!this->unique_materials_buffer) {
	...
	// write Material data to staging buffer
	Task& task = manager.get_graphics_task(__FUNCTION__);
	...
	// copy from staging buffer to device-local SSBO
	CommandBuffer& cb = task.get_command_buffer();
	cb.begin_recording();
	cb.add_buffer_memory_barrier(
		staging_buffer,
...
// returns an SSBO holding the properties of the visible lights of the given Scene object;
// note: this buffer will only update if
// (1) the number of visible lights has changed
// (2) this is the first call for the given Scene object
// (3) the 'force_update' flag is set to 'true'
Buffer<Light>& Scene::get_lights_buffer(Semaphore& timeline_semaphore, bool force_update) {
	VulkanManager& manager = VulkanManager::get_singleton();
	...
	// ensure at least 1 visible light
	if (num_lights == 0) {
		...
		this->add_scene_light(LightType::POINT_LIGHT, { 0,0,0 });
		num_lights++;
	}

	// check if the lights_buffer SSBO needs updating
	if (this->lights_buffer) {

		// return the existing buffer if the number of visible lights hasn't changed
		if (this->lights_buffer->get_elements() == num_lights) {
			if (!force_update) {
				return *this->lights_buffer;
			}
		}
		// release the old SSBO in case the number of visible lights has changed
		else {
			manager.get_device().wait_idle();
			this->lights_buffer.reset(nullptr); // clean up the old resource before proceeding
		}
	}

	Task& task = manager.get_graphics_task(__FUNCTION__);

	// create lights buffer SSBO in case it doesn't exist yet
	if (!this->lights_buffer) {
	...
	CommandBuffer& cb = task.get_command_buffer();
	Buffer<Light>& staging_buffer = task.make_buffer<Light>(
		num_lights,
		BufferType::TRANSFER_BUFFER,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
	);
	
	// ... write scene lights to staging buffer
	uint32_t offset_elements = 0;
	for (auto& scene_light : this->scene_lights) {
		if (scene_light.second->is_visible()) {
			staging_buffer.write({ *scene_light.second }, offset_elements);
			offset_elements++;
		}
	}

	// ... write entity lights to staging buffer
	for (auto& entity : entities) {
		if (entity.second->is_visible()) {
			for (uint32_t i = 0; i < entity.second->get_mesh().get_lights_count(); i++) {
				staging_buffer.write({ entity.second->get_mesh().get_lights()[i] }, offset_elements, GRAPHICS_QUEUE);
				offset_elements++;
			}
		}
	}
...
// returns a device-local SSBO of the model matrices of the visible Entities in the given Scene
Buffer<glm::mat4>& Scene::get_model_matrices_buffer(Semaphore& tl_semaphore) {
	...
	if (this->model_matrices_buffer) {
		// release the old SSBO in case the number of visible entities has changed
	...
	// create the model matrices buffer SSBO in case it doesn't exist yet
	if (!this->model_matrices_buffer) {
	...
	// write model matrices to staging buffer
	Task& task = manager.get_graphics_task(__FUNCTION__);
	CommandBuffer& cb = task.get_command_buffer();
	...
	for (auto& entity : this->entities) {
		if (entity.second->is_visible()) {
			staging_buffer.write({ entity.second->get_model_matrix() }, offset_elements, GRAPHICS_QUEUE);
			offset_elements++;
		}
	}
	...
	// copy from staging buffer to device-local SSBO
	cb.begin_recording();
	cb.add_buffer_memory_barrier(staging_buffer, VK_ACCESS_2_TRANSFER_READ_BIT, VK_ACCESS_2_HOST_WRITE_BIT, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_PIPELINE_STAGE_2_HOST_BIT);
	cb.record_barriers();
...
Buffer<MaterialTexIDs>& Scene::get_scene_texIDs_buffer(Semaphore& tl_semaphore, bool force_update) {

	VulkanManager& manager = VulkanManager::get_singleton();
	uint32_t unique_materials_count = static_cast<uint32_t>(this->unique_materials.size());

	// check if the scene_texIDs_buffer SSBO needs updating
	if (this->scene_texIDs_buffer) {

		// return the existing buffer if the number unique materials hasn't changed
		if (this->scene_texIDs_buffer->get_elements() == unique_materials_count) {
			if (!force_update) {
				return *this->scene_texIDs_buffer;
			}
		}
		// release the old SSBO in case the number of unique materials has changed
		else {
			manager.get_device().wait_idle();
			this->scene_texIDs_buffer.reset(nullptr); // clean up the old resource before proceeding
		}
	}

	// Prepare the CPU-side data array (one struct per unique material)
	std::vector<MaterialTexIDs> scene_texIDs_vec(unique_materials_count);

	// Define the translation lambda: Global ID -> TexturePath -> Scene-Local Index
	auto translate_ID = [&](int32_t global_id) -> int32_t {
		if (global_id < 0) { return -1; /* No texture */ }
	...
	for (size_t i = 0; i < unique_materials_count; i++) {
		auto& src = unique_materials[i]->texIDs;
		auto& dst = scene_texIDs_vec[i];
	...
	// create the device-local SSBO in case it doesn't exist yet
	if (!this->scene_texIDs_buffer) {
		this->scene_texIDs_buffer = std::make_unique<Buffer<MaterialTexIDs>>(
			manager.get_device(),
			BufferType::STORAGE_BUFFER,
	...
	// write the texID results to device-local buffer (via staging buffer)
	this->scene_texIDs_buffer->write(scene_texIDs_vec, 0, 0, 0, tl_semaphore, GRAPHICS_QUEUE);
...
Renderer::Renderer(Scene& scene, VkExtent2D extent, uint32_t frames_in_flight) :
	...
	image_available_semaphore.reserve(frames_in_flight);
	render_finished_semaphore.reserve(frames_in_flight);
	task.reserve(frames_in_flight);
	for (uint32_t i = 0; i < frames_in_flight; i++) {
		image_available_semaphore.emplace_back(vk_manager->get_device(), VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT);
		render_finished_semaphore.emplace_back(vk_manager->get_device(), VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT);
	}
}
...
void Renderer::set_descriptor_set(DescriptorSet& descriptor_set) {
	this->descriptor_set.clear();
	this->descriptor_set.reserve(MAX_FRAMES_IN_FLIGHT);
	this->descriptor_set.push_back(&descriptor_set);
}

void Renderer::set_push_constants(PushConstants& push_constants) {
	this->push_constants.clear();
	this->push_constants.reserve(MAX_FRAMES_IN_FLIGHT);
	this->push_constants.push_back(&push_constants);
}
...
void Renderer::initialize(Semaphore& timeline_semaphore) {
	...
	// (if not: create one that matches the default fragment shader's expectations);
	// multiple copies are created, one per frame in flight;
	if (this->descriptor_set.empty()) {
		is_default_renderer = true;
		this->m_descriptor_set.clear();
		this->m_descriptor_set.reserve(frames_in_flight);
		this->descriptor_set.reserve(frames_in_flight);
		for (uint32_t i = 0; i < frames_in_flight; i++) {
			this->m_descriptor_set.emplace_back(std::make_unique<DescriptorSet>(vk_manager->get_device(), *descriptor_set_layout, vk_manager->get_descriptor_pool()));
			this->m_descriptor_set.back()->bind_buffer(0, scene->get_unique_materials_buffer(timeline_semaphore));
			this->m_descriptor_set.back()->bind_buffer(1, scene->get_lights_buffer(timeline_semaphore));
			this->m_descriptor_set.back()->bind_buffer(2, scene->get_scene_texIDs_buffer(timeline_semaphore));
			this->m_descriptor_set.back()->bind_buffer(3, scene->get_model_matrices_buffer(timeline_semaphore));
			this->m_descriptor_set.back()->bind_cubemap(4, scene->get_cubemap_irradiance(timeline_semaphore));
			this->m_descriptor_set.back()->bind_cubemap(5, scene->get_cubemap_prefiltered(timeline_semaphore));
			this->m_descriptor_set.back()->bind_textures(6, scene->get_brdf_lut(timeline_semaphore));
			this->m_descriptor_set.back()->bind_textures(7, scene->get_textures());
			vk_manager->get_device().wait_idle();
			this->m_descriptor_set.back()->write();
			this->descriptor_set.push_back(this->m_descriptor_set.back().get());
		}
	}
	...
	if (this->push_constants.empty()) {
		is_default_renderer = true;
		this->m_push_constants.clear();
		this->m_push_constants.reserve(frames_in_flight);
		this->push_constants.reserve(frames_in_flight);
		for (uint32_t i = 0; i < frames_in_flight; i++) {
			this->m_push_constants.emplace_back(std::make_unique<PushConstants>());
			default_offsets.view_matrix_offset =			m_push_constants.back()->add_values(scene->get_active_camera().get_view_matrix());
			default_offsets.projection_offset =				m_push_constants.back()->add_values(scene->get_active_camera().get_projection_matrix());
			default_offsets.material_offset =				m_push_constants.back()->add_values(uint32_t(0));
			default_offsets.light_count_offset =			m_push_constants.back()->add_values(scene->get_visible_lights_count());
			default_offsets.prefmipl_offset =				m_push_constants.back()->add_values(scene->get_prefiltered_mip_levels());
			default_offsets.exposure_offset =				m_push_constants.back()->add_values(scene->get_exposure());
			default_offsets.contrast_offset =				m_push_constants.back()->add_values(scene->get_contrast());
			default_offsets.ibl_intensity_offset =			m_push_constants.back()->add_values(scene->get_ibl_intensity());
			default_offsets.ambient_scene_color_offset =	m_push_constants.back()->add_values(scene->get_ambient());
			default_offsets.camera_position_offset =		m_push_constants.back()->add_values(scene->get_active_camera().get_position());
			this->push_constants.push_back(this->m_push_constants.back().get());
		}
	}
...
bool Renderer::render_next_frame(Semaphore& timeline_semaphore) {
	...
	// find "frame in flight" with idle task
	Log::debug("Searching for available frame task in order to render the next frame.");
	uint32_t current_frame = UINT32_MAX;
	for (uint32_t i = 0; i < this->frames_in_flight; i++) {
		if (i >= task.size()) {
			task.emplace_back(vk_manager->get_device(), vk_manager->get_command_pool_graphics(), vk_manager->get_descriptor_pool(), __FUNCTION__);
			current_frame = i;
			break;
		}
		else {
	...
	Log::info(
		...
		frames_in_flight, ", FPS: ", fps, ", Task: ", &task[current_frame], "]"
	);
	Log::debug("\n===============================================================================");
#endif
	...
	if (is_default_renderer) {
		Log::debug("... updating model matrices buffer at descriptor set binding ", default_bindings.model_matrices, ".");
		this->descriptor_set[current_frame]->bind_buffer(default_bindings.model_matrices, this->scene->get_model_matrices_buffer(timeline_semaphore));
	}
	this->descriptor_set[current_frame]->write();
	...
	std::vector<AlphaDrawCall> alpha_queue;

	// update any push constants that may have changed since the last frame
	// (only for the default renderer; custom renderers must handle this on their own before calling this function,
	// because if a non-default fragment shader is used, the order and number of push constants are unknown)
	if (is_default_renderer) {
		Log::debug("... updating push constants");
		this->push_constants[current_frame]->add_values(scene->get_active_camera().get_view_matrix(), default_offsets.view_matrix_offset);
		this->push_constants[current_frame]->add_values(scene->get_active_camera().get_projection_matrix(), default_offsets.projection_offset);
		this->push_constants[current_frame]->add_values(scene->get_visible_lights_count(), default_offsets.light_count_offset);
		//this->push_constants[current_frame]->add_values(scene->get_prefiltered_mip_levels(), default_offsets.prefmipl_offset); // shouldn't change unless the cubemap changes
		this->push_constants[current_frame]->add_values(scene->get_exposure(), default_offsets.exposure_offset);
		this->push_constants[current_frame]->add_values(scene->get_contrast(), default_offsets.contrast_offset);
		//this->push_constants[current_frame]->add_values(scene->get_ibl_intensity(), default_offsets.ibl_intensity_offset); // shouldn't typically change after initial setup
		this->push_constants[current_frame]->add_values(scene->get_ambient(), default_offsets.ambient_scene_color_offset);
		this->push_constants[current_frame]->add_values(scene->get_active_camera().get_position(), default_offsets.camera_position_offset);
	}

	Camera& cam = this->scene->get_active_camera();
	uint32_t image_index = this->swapchain->acquire_next_image(image_available_semaphore[current_frame]);

	CommandBuffer& cb = task[current_frame].get_command_buffer();
	cb.begin_recording();
	cb.set_viewport({ 0, 0 }, extent);
	cb.set_scissor({ 0, 0 }, extent);
	...
	// ======= RENDER OPAQUE PASS ==========
	cb.bind_descriptor_set(*this->descriptor_set[current_frame], *this->pipeline_opaque);
	cb.bind_pipeline(*this->pipeline_opaque);
	Mesh* last_bound_mesh = nullptr;

	for (auto& it : this->scene->get_unique_meshes()) {
		Mesh* current_mesh = it.second;

		for (uint32_t entity_index = 0; entity_index < entities.size(); entity_index++) {
			if (entities[entity_index]->get_mesh().get_unique_ID() == current_mesh->get_unique_ID()) {

				for (const auto& submesh : entities[entity_index]->get_mesh().get_submeshes()) {
					// check if this specific submesh/material is transparent (Even in an "alpha" mesh, some submeshes might be opaque)
					const Material& mat = current_mesh->get_materials()[submesh.scene_local_material_index];
					bool is_transparent =
						mat.alpha_mode == AlphaMode::ALPHA_BLEND_MODE ||
						(mat.alpha_mode == AlphaMode::MASK_MODE && mat.base_color.a <= mat.alpha_cutoff) ||
						mat.transmission_factor > 0.01;

					if (is_transparent) {
						// calculate submesh distance from camera for sorting
						glm::vec3 submesh_world_center = glm::vec3(entities[entity_index]->get_model_matrix() * glm::vec4(submesh.get_center(), 1.0f));
						float dist = cam.get_distance(submesh_world_center);
						alpha_queue.push_back({ dist, entity_index, &submesh, current_mesh });
					}
					else {
						// Render Opaque immediately
						if (last_bound_mesh != current_mesh) {
							cb.bind_mesh(*current_mesh);
							last_bound_mesh = current_mesh;
						}
						if (is_default_renderer) {
							this->push_constants[current_frame]->add_values(submesh.scene_local_material_index, default_offsets.material_offset);
						}
						cb.bind_push_constants(*this->push_constants[current_frame], *this->pipeline_opaque);

						if (submesh.index_count <= 1) {
							cb.draw(submesh.vertex_count, 1, submesh.vertex_offset, entity_index);
						}
						else {
							cb.draw_indexed(submesh.index_count, 1, submesh.first_index, 0, entity_index);
						}
					}
				}
			}
	...
	if (!alpha_queue.empty()) {
		// Sort Back-to-Front
		std::sort(alpha_queue.begin(), alpha_queue.end(), [](const AlphaDrawCall& a, const AlphaDrawCall& b) { return a.distance > b.distance; });

		cb.bind_descriptor_set(*this->descriptor_set[current_frame], *this->pipeline_alpha);
		cb.bind_pipeline(*this->pipeline_alpha);

		for (const auto& draw_call : alpha_queue) {
			if (draw_call.mesh != last_bound_mesh) {
				cb.bind_mesh(*draw_call.mesh);
				last_bound_mesh = draw_call.mesh;
			}

			if (is_default_renderer) {
				this->push_constants[current_frame]->add_values(draw_call.submesh->scene_local_material_index, default_offsets.material_offset);
			}
			cb.bind_push_constants(*this->push_constants[current_frame], *this->pipeline_alpha);

			if (draw_call.submesh->index_count <= 1) {
				cb.draw(draw_call.submesh->vertex_count, 1, draw_call.submesh->vertex_offset, draw_call.entity_index);
			}
			else {
				cb.draw_indexed(draw_call.submesh->index_count, 1, draw_call.submesh->first_index, 0, draw_call.entity_index);
			}
		}
	}
	...
	cb.end_renderpass();
	cb.end_recording();
	task[current_frame].timeline_sync(timeline_semaphore);
	task[current_frame].wait_binary_semaphore(image_available_semaphore[current_frame]);
	task[current_frame].signal_binary_semaphore(render_finished_semaphore[current_frame]);
	task[current_frame].submit();

	this->swapchain->present_rendered_image(render_finished_semaphore[current_frame]);
	Log::debug("===============================================================================\n\n");
	return true;
}
...
PushConstants& Renderer::get_constants() { if (push_constants.empty()) { Log::warning("in Renderer::get_push_constants(): push_constants is NULL."); }; return *this->push_constants[total_frames_counter % frames_in_flight]; }
DescriptorSet& Renderer::get_descriptor_set() { if (descriptor_set.empty()) { Log::warning("in Renderer::get_descriptor_set(): descriptor_set is NULL."); }; return *this->descriptor_set[total_frames_counter % frames_in_flight]; }
