#ifndef GRAPHICS_TEST_CHRONOGRAPH_SIMPLIFIED_H
#define GRAPHICS_TEST_CHRONOGRAPH_SIMPLIFIED_H

#include <vkcontext.h>

void graphics_test_chronograph() {
	...
	Surface& surface = manager.get_surface(extent.width, extent.height, "Vulkan Graphics Test - Chronograph Watch (Khronos Group)");

	// load glTF model and add as scene entity
	Mesh model(device, "resources/models/gltf/ChronographWatch.glb", tl_semaphore, true, true, 1); // source: Khronos Group
