...
namespace cdf {
    ...
    template<typename T>
    T gaussian(T x_val, T mu, T sigma) {
        return 0.5 * (1 + erf((x_val - mu) / (sigma * SQRT_2)));
    }
    ...
    // being less or equal than the given 'x_val'
    template<typename T>
    T cauchy(T x_val, T x_peak, T gamma) {
        return 0.5 + atan((x_val - x_peak) / gamma) / PI;
    }
    ...
    // note: not defined for x<=0!!
    template<typename T>
    T lomax(T x_val, T alpha, T tail_index) {
        return 1 - pow(1 + std::fmax(0, x_val) / tail_index, -alpha);
    }


    template<typename T>
    T F_distribution(T x_val, T d1, T d2) {
        if (x_val < 0) {
            return 0;
        }
